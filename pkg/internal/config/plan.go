...
func (c *ProjectConfig) Plan(configDir, workflow string, vars map[string]string, ov run.PlanOverrides) (*run.Plan, error) {
	...
	return &run.Plan{
		Config: workshop.Config{
			Workshop:         workshop.WorkshopName(c.Workshop, profile.Name()),
			Base:             c.Base,
			Agent:            profile,
			RepoPath:         repoPath,
		...
		Request: run.OrchestratedRequest{
			RunRequest:       run.RunRequest{Branch: branch, Prompt: promptText, Timeout: timeout, Stdout: stdout, Stderr: stderr},
