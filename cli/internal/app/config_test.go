...
// test's working directory that holds workshop.yaml. That directory is itself a
...
// rejects, so it can never be a "fully clean" repo). The marker is workshop.yaml
// rather than go.mod because the three-module split puts a go.mod in cli/ too, so
// go.mod no longer uniquely identifies the repo root. A missing root is a hard
// t.Fatal, never a silent skip. Mirrors findRepoWorkshopYAML in pkg/taboo.
func tabooRepoRoot(t *testing.T) string {
	...
	for {
		...
		if parent == dir {
			t.Fatal("could not locate taboo repo root (no workshop.yaml above the test working dir)")
		}
		dir = parent
	}
}
