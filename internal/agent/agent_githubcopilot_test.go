...
func TestCopilot_Name(t *testing.T) {
	if got := NewGitHubCopilot(copilotModel).Name(); got != GitHubCopilot {
		t.Errorf("Name() = %q, want %q", got, GitHubCopilot)
	}
}

func TestCopilot_BuildCommand(t *testing.T) {
	ac := NewGitHubCopilot(copilotModel).BuildCommand(CommandOptions{Prompt: "do the thing"})
...
func TestCopilot_BuildCommand_Resume(t *testing.T) {
	ac := NewGitHubCopilot(copilotModel).BuildCommand(CommandOptions{
...
func TestCopilot_BuildCommand_UsesModel(t *testing.T) {
	ac := NewGitHubCopilot("claude-sonnet-4.6").BuildCommand(CommandOptions{Prompt: "go"})
...
func TestCopilot_BuildCommand_ForkIsNoOp(t *testing.T) {
	resume := NewGitHubCopilot(copilotModel).BuildCommand(CommandOptions{
		Prompt: "do the thing", ResumeSession: "ses_abc",
	})
	fork := NewGitHubCopilot(copilotModel).BuildCommand(CommandOptions{
...
func TestCopilot_BuildCommand_ForkWithoutResumeIgnored(t *testing.T) {
	ac := NewGitHubCopilot(copilotModel).BuildCommand(CommandOptions{Prompt: "go", Fork: true})
...
func TestCopilot_BuildCommand_EmptyPrompt(t *testing.T) {
	ac := NewGitHubCopilot(copilotModel).BuildCommand(CommandOptions{ResumeSession: "ses_abc"})
...
func TestCopilot_CredentialEnvKeys(t *testing.T) {
	got := NewGitHubCopilot(copilotModel).CredentialEnvKeys()
...
func TestCopilot_Sessions(t *testing.T) {
	spec, ok := NewGitHubCopilot(copilotModel).Sessions()
	if !ok {
