...
//
//	TABOO_BENCH=1 go test -tags integration ./pkg/ \
...
func (p *phaseTimer) Run(ctx context.Context, c exec.Cmd) error {
	start := time.Now()
...
func TestBenchmark_PoolFanoutColdStart(t *testing.T) {
	...
	pt := &phaseTimer{inner: exec.NewExecCommander()}
...
func cleanupPoolWorkshops(t *testing.T, proj, ws string, slots int) {
	...
	for i := 0; i < slots; i++ {
		...
		_ = exec.NewExecCommander().Run(context.Background(), exec.Cmd{
