...
The seeded `.taboo/taboo.yaml` looks like this. The `workshop`, `base`, `repo`, `agent`, and `model` values come from your flags or wizard answers; only `strategy` is fixed to `worktree` by the scaffold. The values below are examples:
