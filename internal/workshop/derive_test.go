...
// root. The test CWD is <repo>/internal/workshop, so it walks up from the
...
func TestDeriveDefinition_RejectsNonMappingSource(t *testing.T) {
	cfg := Config{Workshop: "w", Base: "ubuntu@24.04", Agent: mustProfile("opencode", openCodeModel), RepoPath: "/r"}
...
// under pkg/sdk/. Its other methods return zero values; only Name() drives
// the embedded-SDK check.
type ghostProfile struct{}

func (ghostProfile) Name() agent.AgentName { return "ghost" }
func (ghostProfile) BuildCommand(agent.CommandOptions) agent.AgentCommand {
	return agent.AgentCommand{}
}
func (ghostProfile) CredentialEnvKeys() []string         { return nil }
func (ghostProfile) Sessions() (agent.SessionSpec, bool) { return agent.SessionSpec{}, false }
