...
func TestHookCmd(t *testing.T) {
	sessionEnv := []workshop.EnvAssignment{{Name: "XDG_DATA_HOME", Value: workshop.SessionsTarget}}
	...
	want := workshop.ExecArgs("/proj", "ws",
		workshop.ExecOptions{Cwd: workshop.WorkspaceTarget, Timeout: 30 * time.Second, EnvKeys: []string{"OPENROUTER_API_KEY"}, Env: sessionEnv},
		[]string{"go", "build"})
...
func TestRun_OnWorkshopReadyInWorkshopHookInheritsAgentContext(t *testing.T) {
	...
	idx := indexOfExecContaining(fc, "download")
	want := workshop.ExecArgs(cfg.ProjectDir, cfg.Workshop,
		workshop.ExecOptions{
			Cwd:     workshop.WorkspaceTarget,
			EnvKeys: cfg.Agent.CredentialEnvKeys(),
			Env:     []workshop.EnvAssignment{{Name: spec.DirEnv, Value: workshop.SessionsTarget}},
		},
