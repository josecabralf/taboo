...
func TestRun_PerRunSequence(t *testing.T) {
	...
	wantWs := workshop.RemountArgs(cfg.ProjectDir, cfg.Workshop, cfg.Agent.Name(), "workspace", res.WorktreePath)
	...
	wantGc := workshop.RemountArgs(cfg.ProjectDir, cfg.Workshop, cfg.Agent.Name(), "gitcommon", workshop.GitCommonTarget(cfg.RepoPath))
	...
	wantSess := workshop.RemountArgs(cfg.ProjectDir, cfg.Workshop, cfg.Agent.Name(), "sessions", filepath.Join(cfg.ProjectDir, "sessions"))
