...
func TestWriteDefinition_DerivesFromProjectDef(t *testing.T) {
	...
	for _, s := range got.SDKs {
		...
		case projectSDKRef(string(cfg.Agent.Name())): // "project-opencode"
			agentSDK = true
		}
	}
	if !goSDK {
	...
	if !agentSDK {
		t.Errorf("agent SDK %q not appended; sdks = %+v", projectSDKRef(string(cfg.Agent.Name())), got.SDKs)
	}
...
func TestSeedSDK_OnlyConfiguredAgent(t *testing.T) {
	...
	for _, name := range agent.AgentNames() {
		...
		t.Run(string(name), func(t *testing.T) {
			p, err := agent.NewProfile(agent.AgentName(name), "")
			if err != nil {
			...
			sdkYAML := filepath.Join(cfg.ProjectDir, ".workshop", string(name), "sdk.yaml")
