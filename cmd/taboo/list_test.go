...
// listFakeStdout programs the fake commander with the canned host stdout the
// list probes parse: a realistic `workshop info` YAML block (so the status
// parses to "ready") for any workshop-info probe, and empty otherwise. The probe
// arg is now a derived per-agent name ("demo-opencode"), not the bare base, so
// the match is on the "info" verb alone; the YAML name field is irrelevant since
// list prints names from projectWorkshops and parses only the status. It is
// extended each cycle as list grows new probes (git worktree porcelain, git
// for-each-ref lines).
func listFakeStdout(c taboo.Cmd) string {
	...
	if c.Name == "git" && elemsContain(c.Args, "worktree", "list", "--porcelain") {
		...
		managed := filepath.Join(listFakeRoot, ".taboo", "worktrees", "taboo-fix-123")
...
// listFakeRoot is the temp project root the worktree-porcelain canned stdout is
// built against. Each test that exercises the worktrees section sets it before
// calling listCmd so the managed worktree lands under <root>/.taboo/worktrees.
var listFakeRoot string
...
// TestList_WorkshopState is the tracer bullet: list discovers the project
// config, probes the configured workshop's state via `workshop --project
// <projectDir> info <name>`, and reports the workshop and its parsed status.
func TestList_WorkshopState(t *testing.T) {
	root := t.TempDir()
	writeTabooProject(t, root, listProjectBody)
	fake := &fakeCommander{stdoutFn: listFakeStdout}
