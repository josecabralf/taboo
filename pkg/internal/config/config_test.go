...
`+tt.workflow)
			...
			argv := wf.Profile.BuildCommand(agent.CommandOptions{Prompt: "x"}).Argv
...
// TestResolveTimeout covers the workflow-then-defaults precedence: the workflow
// value wins when set, the defaults fill in otherwise, and a nil defaults block
// yields the zero value (no timeout).
func TestResolveTimeout(t *testing.T) {
	t.Parallel()

	defaults := &RunDefaults{Timeout: Duration(10 * time.Minute)}
	tests := []struct {
		name     string
		wf       Workflow
		...
		{"workflow wins", Workflow{Timeout: Duration(5 * time.Minute)}, defaults, 5 * time.Minute},
		{"defaults fallback", Workflow{}, defaults, 10 * time.Minute},
		{"nil defaults is zero", Workflow{}, nil, 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := ResolveTimeout(tt.wf, tt.defaults); got != tt.want {
...
// TestResolveMaxIterations covers the workflow-then-defaults precedence for the
// iteration cap: the workflow value wins when positive, the defaults fill in
// otherwise, and a nil defaults block yields zero.
func TestResolveMaxIterations(t *testing.T) {
	t.Parallel()

	defaults := &RunDefaults{MaxIterations: 3}
	tests := []struct {
		name     string
		wf       Workflow
		defaults *RunDefaults
		...
		{"workflow wins", Workflow{MaxIterations: 7}, defaults, 7},
		{"defaults fallback", Workflow{}, defaults, 3},
		{"nil defaults is zero", Workflow{}, nil, 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			if got := ResolveMaxIterations(tt.wf, tt.defaults); got != tt.want {
				t.Errorf("ResolveMaxIterations = %d, want %d", got, tt.want)
			}
		})
	}
}

// TestResolveCompletionSignal covers the single-layer lookup: the defaults'
// signal when a block is set, "" when defaults is nil.
func TestResolveCompletionSignal(t *testing.T) {
	t.Parallel()

	if got := ResolveCompletionSignal(&RunDefaults{CompletionSignal: "DONE"}); got != "DONE" {
		t.Errorf("ResolveCompletionSignal = %q, want %q", got, "DONE")
	}
	if got := ResolveCompletionSignal(nil); got != "" {
		t.Errorf("ResolveCompletionSignal(nil) = %q, want empty", got)
	}
}
