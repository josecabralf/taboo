...
// libraryVersion is the pkg/taboo version the scaffolded go.mod pins to. Release
// tooling overwrites it via -ldflags. The `v0.0.0` default marks an unreleased dev
// build and is kept valid semver so the require directive is well-formed.
...
// scaffoldFile is one file in the scaffold plan.
...
// plan renders the scaffold files in stable order: the three base files, then the
// seeded prompt files when SeedWorkflows is set, then the Go scaffold when a
// Template is chosen. Only taboo.yaml, which marshals YAML, can fail.
...
// variant fans several runs across a taboo.NewPool, otherwise the single-run
// program.
...
// as a richer skeleton (see github.com/josecabralf/taboo). It reads the
...
// fan-out or structured output (see github.com/josecabralf/taboo). It
...
// renderGoMod returns the scaffolded go.mod: the module named after the workshop,
// the taboo library pinned to the exact libraryVersion so the scaffold is
// reproducible.
func renderGoMod(in scaffoldInputs) []byte {
	...
	buf.WriteString(in.Workshop)
	buf.WriteString("\n")
	buf.WriteString("\n")
	buf.WriteString("go " + scaffoldGoVersion + "\n")
	buf.WriteString("\n")
	buf.WriteString("require github.com/josecabralf/taboo ")
	buf.WriteString(libraryVersion)
	buf.WriteString("\n")
	return buf.Bytes()
}

// fixPromptBody is the prompt seeded at prompts/fix.md.
...
// refactorPromptBody is the prompt seeded at prompts/refactor.md.
...
// When SeedWorkflows is set it marshals a real fix/refactor workflows block; when
// not, it appends a commented footer showing where the optional blocks go.
...
// renderGitignore returns the .taboo/.gitignore body.
func renderGitignore() []byte {
	...
	buf.WriteString("/workshop.fingerprint\n") // the digest of the def the live workshop was last provisioned with (anchored, beside the derived workshop.yaml)
	buf.WriteString(".env\n")
	buf.WriteString("logs/\n")
	buf.WriteString("sessions/\n") // the branch strategy writes a session-capable agent's state here, inside the checkout; never commit it, and never let it dirty the clean-checkout guard
	return buf.Bytes()
}

// renderEnvExample returns the .taboo/.env.example body: a header naming the
// chosen agent, then one KEY= line per credential env key the agent reads.
func renderEnvExample(in scaffoldInputs) []byte {
	var buf bytes.Buffer
	buf.WriteString("# .env.example — generated by `taboo init` for agent ")
	buf.WriteString(string(in.Agent))
	buf.WriteString(".\n")
...
// writeScaffold creates projectDir and writes every planned file under it. Dirs
// are 0750 and files 0600 per the repo's gosec posture.
