...
The issue above is a PRD-style brief. Break it into a set of **vertical-slice child issues**. Each child must be independently grabbable by an agent: it delivers a coherent, testable increment on its own, touches as few of the same files as its siblings as possible, and carries enough context to be implemented without re-reading the PRD.
...
- Keep every `blocked_by` index strictly backward-pointing. The orchestrator drops any index that is not a smaller position in this array (a forward, self, or out-of-range reference) and prints a notice.
